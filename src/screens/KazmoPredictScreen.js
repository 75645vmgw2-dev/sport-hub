    ...
    return(
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.analysisTabs}>{[{id:'rapide',label:'⚡ '+t('quick')},{id:'detaille',label:'📊 '+t('detailed')},{id:'expert',label:'🎓 '+t('expert')}].map(function(tb){return(<TouchableOpacity key={tb.id} style={[styles.analysisTab,analysisTab===tb.id&&styles.analysisTabActive]} onPress={()=>setAnalysisTab(tb.id)}><Text style={[styles.analysisTabText,analysisTab===tb.id&&{color:'#FF6B2B'}]}>{tb.label}</Text></TouchableOpacity>);})}</View>
          <View style={styles.analysisContent}><Text style={styles.analysisText}>{(analysisTab==='rapide'?result.rapide:analysisTab==='detaille'?result.detaille:result.expert||'').replace(/<cite[^>]*>/g,'').replace(/<\/cite>/g,'').replace(/\[\d+\]/g,'')}</Text></View>
